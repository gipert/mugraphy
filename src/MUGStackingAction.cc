...
MUGStackingAction::MUGStackingAction(MUGEventAction* eventaction) : fEventAction(eventaction) {
...
void MUGStackingAction::BeginOfRunAction() {
  if (fKillSecondaries)
    MUGLog::Out(MUGLog::detail, "µ-induced secondary particles will *not* be tracked");
  else MUGLog::Out(MUGLog::detail, "µ-induced secondary particles will be tracked");
}

void MUGStackingAction::DefineCommands() {

  fMessenger = std::make_unique<G4GenericMessenger>(this, "/MUG/Stacking/",
