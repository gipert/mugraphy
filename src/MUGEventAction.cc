...
MUGEventAction::MUGEventAction(MUGRunAction* run_action) : fRunAction(run_action) {
  this->DefineCommands();
}
...
void MUGEventAction::EndOfEventAction(const G4Event* event) {
  ...
  auto hit_coll =
      dynamic_cast<MUGPanelHitsCollection*>(event->GetHCofThisEvent()->GetHC(hit_coll_id));
  if (!hit_coll) {
  ...
  if (MUGManager::GetMUGManager()->IsPersistencyEnabled()) {
    ...
    for (auto hit : *hit_coll->GetVector()) {
      ...
      fRunAction->GetThetaVec().push_back(
          (CLHEP::pi - hit->GetMomDir().getTheta()) / G4Analysis::GetUnitValue("rad"));
      fRunAction->GetPhiVec().push_back(
          hit->GetMomDir().getPhi() / G4Analysis::GetUnitValue("rad"));
    }
