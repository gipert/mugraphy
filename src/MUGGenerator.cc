...
void MUGGenerator::BeginOfRunAction() {
  ...
  if (sky_height < world_side / 2) {
    MUGLog::Out(MUGLog::fatal,
        "Sky is not high enough, it does not contain the full muon generation region");
  }
  ...
  switch (fSkyShape) {
    ...
    case SkyShape::kSphere: {
      ...
      fEcoMug->SetHSphereCenterPosition(
          {0, 0, (ground_depth - (sky_height + ground_depth) / 2) / u::m});
...
void MUGGenerator::SetSkyShape(std::string shape) {
  try {
    fSkyShape = MUGTools::ToEnum<MUGGenerator::SkyShape>(shape, "sky shape");
