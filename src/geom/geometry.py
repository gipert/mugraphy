...
world_s = geant4.solid.Box(
    "WorldAir", world_side, world_side, world_height, reg, lunit="m"
)
...
# ground
ground_s = geant4.solid.Box(
    "Ground", world_side - eps, world_side - eps, ground_depth - eps, reg, lunit="m"
)
ground_l = geant4.LogicalVolume(ground_s, rock_mat, "Ground", reg)
geant4.PhysicalVolume(
    [0, 0, 0],
    [0, 0, -world_height / 2 + ground_depth / 2 + eps / 2, "m"],
    ground_l,
    "Ground",
    world_l,
    reg,
...
pyramid_s = geant4.solid.Trd(
    "Pyramid", p_side, dl, p_side, dl, p_height, reg, lunit="m"
)
...
if enable_pyramid:
    geant4.PhysicalVolume(
        [0, 0, 0],
        [0, 0, -world_height / 2 + ground_depth + p_height / 2 + eps, "m"],
        pyramid_l,
        "Pyramid",
        world_l,
        reg,
...
if enable_cavities:
    ...
    geant4.PhysicalVolume(
        [0, 0, 0],
        [0, 1.8, 23.3 + 4.7 / 2 + 1.5 / 2 + eps + p_off, "m"],
        queen_hat_l,
        "QueenHat",
        pyramid_l,
        reg,
    ...
    king_cham_dy = 10.6
    king_cham_dz = 5.7
    king_cham_s = geant4.solid.Box(
        "KingChamber", king_cham_dx, king_cham_dy, king_cham_dz, reg, lunit="m"
    )
    king_cham_l = geant4.LogicalVolume(king_cham_s, "G4_AIR", "KingChamber", reg)
    geant4.PhysicalVolume(
        [0, 0, 0],
        [-10.9, 4.7, 42.8 + p_off, "m"],
        king_cham_l,
        "KingChamber",
        pyramid_l,
        reg,
    ...
    for i in range(len(king_buff_z)):
        king_buff_s = geant4.solid.Box(
            "KingChamberAirBuffer" + str(i),
            king_cham_dx,
            king_cham_dy,
            king_buff_dz[i],
            reg,
        ...
        king_buff_l = geant4.LogicalVolume(
            king_buff_s, "G4_AIR", "KingChamberAirBuffer" + str(i), reg
        )
        geant4.PhysicalVolume(
            [0, 0, 0],
            [-10.9, 4.7, 42.8 + king_cham_dz / 2 + king_buff_z[i] + p_off, "m"],
            king_buff_l,
            "KingChamberAirBuffer" + str(i),
            pyramid_l,
            reg,
    ...
    # King's chamber hat
    king_hat_s = geant4.solid.Trd(
        "KingHat", 6.2, 0, king_cham_dy, king_cham_dy, 2.6, reg, lunit="m"
    )
    king_hat_l = geant4.LogicalVolume(king_hat_s, "G4_AIR", "KingHat", reg)
    geant4.PhysicalVolume(
        [0, 0, 0],
        [-10.9, 4.7, 42.8 + king_cham_dz / 2 + 13.9 + p_off, "m"],
        king_hat_l,
        "KingHat",
        pyramid_l,
        reg,
    ...
    grand_gallery_s = geant4.solid.Para(
        "GrandGallery",
        47.2,
        2.1,
        7.7,
        0,
        -grand_gallery_slope * np.pi / 180,
        0,
        reg,
        lunit="m",
    )
    grand_gallery_l = geant4.LogicalVolume(
        grand_gallery_s, "G4_AIR", "GrandGallery", reg
    )
    geant4.PhysicalVolume(
...
geant4.PhysicalVolume(
    [0, 0, 0], queen_cham_floor, det_l, "LowerPanel", mother, reg, copyNumber=0
)
